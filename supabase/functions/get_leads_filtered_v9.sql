-- MASTER RPC: get_leads_filtered_v9
-- Extends v8 with pipeline_stage multi-value filter (p_pipeline_stage INTEGER[]).
-- All other params and behaviour are identical to v8.

DROP FUNCTION IF EXISTS public.get_leads_filtered_v9;

CREATE OR REPLACE FUNCTION public.get_leads_filtered_v9(
    ...
    -- NEW in v9
    p_pipeline_stage INTEGER[] DEFAULT NULL,
    ...
    WITH filtered_leads AS (
        ...
        WHERE
            ...
            AND (
                ...
                OR
                -- Allow unassigned leads to be visible when the unassigned filter is active
