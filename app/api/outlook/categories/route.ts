...
export async function GET(_request: NextRequest) {
  try {
    ...
    if (userError) {
      console.error('[Outlook Categories] Failed to load user session:', userError)
    ...
    if (profileError) {
      console.error('[Outlook Categories] Failed to load user profile:', profileError)
    ...
    const categories = await fetchOutlookCategories(accessToken)

    return NextResponse.json({ categories })
  } catch (error) {
    console.error('[Outlook Categories] Unexpected error:', error)
